...
`AVFormatContext `是API中直接接触到的结构体，位于`avformat.h`中，是音视频数据,也就是音视频文件（通常接触到的mp3/mp4等文件）的一种抽象和封装，该文件中包含了多路流，包括音频流、视频流、字幕流等。该结构体的使用，贯穿了ffmpeg使用的整个流程，现在数据为王，不是没有道理的。
